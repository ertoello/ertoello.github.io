...
export default function Language() {
  ...
  return (
    <section data-aos='fade-up' data-aos-delay='400' className='flex flex-wrap md:flex-nowrap items-center justify-center gap-10 md:gap-20 h-[300px] text-[#3FA3CE] mt-11 '>
        <div className='grid grid-cols-2 gap-10 md:flex md:gap-20'>
            {
                languages.map((language, index)=>(
                    <div key={index} className='achievement flex flex-col items-center'>
                        ...
                        <span className='text-xl md:text-xl text-[#FFFFFF]'>{language.name}</span>
                        <span className='text-[#EF8B8B] text-lg md:text-xl font-bold'>{language.level}</span>
                    </div>
                ))
            }
        </div>
